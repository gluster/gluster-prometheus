...
if [[ ${EXPORTER_TEST_EXITFIRST} != "yes" && -z ${EXPORTER_TEST_COVERAGE} ]]; then
...
# at at time
if [[ ${EXPORTER_TEST_COVERAGE} ]]; then
...
for gopackage in ${GOPACKAGES}; do
	...
	if [[ ${EXPORTER_TEST_COVERAGE} = "stdout" && -f cover.out ]]; then
		go tool cover -func=cover.out
	fi
	if [[ ${EXPORTER_TEST_COVERAGE} = "html" && -f cover.out ]]; then
		mkdir -p coverage
	...
	rm -f cover.out
	if [[ ${failed} -ne 0 && ${EXPORTER_TEST_EXITFIRST} = "yes" ]]; then
		exit ${failed}
	fi
done
exit ${failed}
